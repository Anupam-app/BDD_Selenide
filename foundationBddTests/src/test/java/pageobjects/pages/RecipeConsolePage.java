...
public class RecipeConsolePage {
	...
	private final SelenideElement postRun_RUNID = $(By.xpath("(//div[@class='postrunFormDetails']/span/label)[1]"));
	private final SelenideElement postRun_BATCHID = $(By.xpath("(//div[@class='postrunFormDetails']/span/label)[3]"));
	...
	public void clickOnJumpToStep(String stepNumber) {
		...
		okStepButton.waitUntil(Condition.visible, 5000l, 500).click();
	...
	public void preRunWindow_Popup() {
		SelenideHelper.commonWaiter(preRun_WindowPopop, visible);
	}
	...
	public void recipenameDisplayed() {
	...
	public void iVerifyTrimmedRecipe() {
		SelenideElement recipelenthName = $(By.xpath("//label[text()='testDraftRecipeToChangeStatus']"));
	...
	public void iCheckStepDetailsWithMouseHover() {
		SelenideElement BatchIdValue = $(By.xpath("(//label[@id='trimString'])[3]"));
		stepAction.moveToElement(BatchIdValue).build().perform();
		Selenide.sleep(2000);
		SelenideElement RunIdValue = $(By.xpath("(//label[@id='trimString'])[4]"));
		stepAction.moveToElement(RunIdValue).build().perform();
		Selenide.sleep(2000);
	}

	public boolean iVerifyRecipeExecution() {
		boolean isResult = false;
		if(preRun_WindowPopop.isDisplayed()) {
			isResult = false;
		}else
