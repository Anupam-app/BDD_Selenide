...
public class ReportsPageStepsDefinition {
	...
	@Then("I verify {string} consolidate summary report")
	public void i_verify_consolidated_summary_report(String value) throws Exception {
		
		/*
		 * this.report.validateConsolidateRunSummary(reportPage.getPdfUrl(),this.recipe.
		 * getMachineName(),
		 * this.recipe.getBatchId(),this.recipe.getProductId(),this.recipe.getRecipeName
		 * (),
		 * this.recipe.getBeforeComments(),this.recipe.getAfterComments(),this.recipe.
		 * getStartDate(),
		 * this.recipe.getEndDate(),this.recipe.getBatchId2(),this.recipe.getProductId()
		 * , this.recipe.getRunId(),this.recipe.getRunId2(),this.recipe.getRunId3());
		 */
		if(value.equalsIgnoreCase("same"))
		{
			/* passing RunId3*/
		  this.report.validateConsolidateRunSummary(reportPage.getPdfUrl(),this.recipe.getMachineName(),this.recipe.getBatchId(),this.recipe.getProductId(),this.recipe.getRecipeName(),
				  this.recipe.getBeforeComments(),this.recipe.getAfterComments(),this.recipe.getStartDate(),this.recipe.getEndDate(),
				  this.recipe.getBatchId2(),this.recipe.getProductId2(),this.recipe.getRunId(),this.recipe.getRunId2(),this.recipe.getRunId3());
	...
	@When("I choose {string} BatchID from Consolidation run")
	public void iChooseRecipeRunForConsolidation(String value) {
		String runIDValue = null;
		if(value.equalsIgnoreCase("different"))
		{
			runIDValue = this.recipe.getRunId2();
		}
		else if(value.equalsIgnoreCase("same"))
		{
			runIDValue = this.recipe.getRunId3();
		}
		reportPage.selectForConsolidationRun(this.recipe.getRunId());
		reportPage.selectForConsolidationRun(runIDValue);
	}
